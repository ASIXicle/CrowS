...
void MadgwickQuaternionUpdate(vec4f& q, float deltat, float beta, float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz){
	...
	if(norm == 0.0f){ // handle NaN
		Serial.println("Accel norm is 0");
		return;
	}
	norm = 1.0f / norm;
	ax *= norm;
...
// measured ones.
/*void MahonyQuaternionUpdate(vec4f& q, float deltat, float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz){
	float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
	float norm;
	float hx, hy, bx, bz;
