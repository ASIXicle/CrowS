...
//----------------------------------------------------------------------------
  ...
  void Bus_SPI::writePixels(pixelcopy_t* param, std::uint32_t length)
  {
    ...
    auto spi_w0_reg = reinterpret_cast<volatile std::uint32_t*>(&SPI1W0);

    std::uint32_t user_reg = SPI1U;
  ...
  {
/*
    if (reinterpret_cast<const std::uintptr_t>(data) & 3)
    {
      auto len = reinterpret_cast<const std::uintptr_t>(data) & 3;
      length -= len;
      std::uint32_t val = 0;
      std::size_t i = 0;
      do
...
//*/
  ...
  {
    std::uint32_t len1 = std::min(length, 32u);  // 32 Byte read.
    std::uint32_t len2 = len1;
    wait_spi();
    set_read_len(len1 << 3);
    exec_spi();
    std::uint32_t highpart = 8;
    std::uint32_t userreg = SPI1U;
    auto spi_w0_reg = reinterpret_cast<volatile std::uint32_t*>(&SPI1W0);
    do {
      if (0 == (length -= len1)) {
        len2 = len1;
